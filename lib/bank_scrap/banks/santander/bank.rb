...
module BankScrap::Banks
  module Santander
    class Bank < BankScrap::Bank
      ...
      # Returns an array of BankScrap::Banks::Santander::Account objects
      def fetch_accounts
        log 'fetch_accounts'
        ...
        response = with_headers(headers) { post(BASE_ENDPOINT + PRODUCTS_ENDPOINT, xml_products) }
      ...
      #
      # Account should be a BankScrap::Banks::Santander::Account object
      ...
      def fetch_transactions_for(account, start_date: Date.today - 1.month, end_date: Date.today)
        ...
        until end_page
          response = with_headers(headers) { post(BASE_ENDPOINT + ACCOUNT_ENDPOINT, xml_account(account, start_date, end_date, repo, importe_cta)) }
          document = parse_context(response)
      ...
      def login
        ...
        response = with_headers(headers) { post(BASE_ENDPOINT + LOGIN_ENDPOINT, xml_login) }
