require 'open-uri'
require 'nokogiri'
require_relative '../accounts/openbank_account'
...
module BankScrap
  class Openbank < Bank
    ...
    def fetch_accounts
      ...
      response = post(BASE_ENDPOINT + PRODUCTS_ENDPOINT, fields: xml_products)
    ...
    def fetch_transactions_for(account, start_date: Date.today - 1.month, end_date: Date.today)
      ...
      until end_page
        response = post(BASE_ENDPOINT + ACCOUNT_ENDPOINT, fields: xml_account(account, start_date, end_date, repo, importe_cta))
        document = parse_context(response)
    ...
    def login
      log 'login'
      response = post(BASE_ENDPOINT + LOGIN_ENDPOINT, fields: xml_login)
